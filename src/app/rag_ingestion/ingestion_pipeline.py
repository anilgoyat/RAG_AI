...
from src.app.rag_ingestion.base_loader import DocumentLoaderFactory
from src.app.rag_ingestion.chunk_strategies import RecursiveCharacterChunkStrategy
from src.app.rag_ingestion.embedding_service import EmbeddingService
from src.app.rag_ingestion.vectordb_factory import VectorDBFactory, VectorStore
...
class IngestionPipeline:
    ...
    def run(self, data_dir: str) -> None:
        ...
        # __ ADDITIONAL bm25 sEARCH
        try:

            logger.info("[Step 3/5] Saving BM25 corpus...")

            bm25_path = Path("vectorstore/faiss_index/documents.pkl")

            bm25_path.parent.mkdir(
                parents=True,
                exist_ok=True
            )
            ...
            logger.info(
                f"[Step 3/5] BM25 corpus saved successfully "
                f"({len(chunks)} chunks)."
            )
            ...
            raise RuntimeError(
                f"Pipeline failed at Step 3 (BM25 Save): {e}"
            ) from e        
